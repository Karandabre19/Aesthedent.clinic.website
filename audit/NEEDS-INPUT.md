...
| N3 | ~~`10+ Years Experience`~~ → **`10+ Years / Experience`** | `lib/clinic.ts` `STATS.years` | ✅ **SOURCED 2026-08-08** - confirmed by Dr. Sahil. Now rendered from `STATS.years`, not hardcoded. **The ambiguity flagged here is still worth resolving:** this reads as Dr. Sahil's years in practice, not the clinic's age - the domain is months old. If that reading is wrong, reword the label. Tracked at C5. |
| N4 | ~~`5000+ Happy Patients`~~ → **`1000+ Happy / Patients`** | `lib/clinic.ts` `STATS.patients` | ✅ **SOURCED 2026-08-08** - confirmed by Dr. Sahil, and note the figure came down 5× from the unsourced original. Now rendered from `STATS.patients`. |
| N5 | ~~`500+ successful cases`~~ → **`100+ Implants / Placed`**, **`500+ Root Canals / Completed`** | `lib/clinic.ts` `STATS.implants`, `STATS.rootCanals` | ✅ **RESOLVED 2026-08-08** - the vague "cases" figure is replaced by two specific, separately-confirmed procedure counts from Dr. Sahil. `rootCanals` is a **count**, labelled "Root Canals / Completed"; it must never be rendered as "Painless Root Canals" (that is the N6 outcome-claim problem). |
...
| C5 | Years in practice, each doctor - ⚠️ **PARTIALLY ANSWERED 2026-08-08:** `10+` confirmed by Dr. Sahil and live as `STATS.years`, but it is a single clinic-wide figure. Whose years it counts, and Dr. Aishwarya's separately, are **still open** (N3). |
| C6 | Implant/FMR case counts, if either is to be claimed - ✅ **ANSWERED 2026-08-08:** `100+` implants placed, `500+` root canals completed, `1000+` patients, all confirmed by Dr. Sahil and live in `lib/clinic.ts` `STATS` (resolves N4/N5). FMR count still unclaimed and unneeded. |
...
| **C12** | **Phase 4G — four corrections applied to Dr. Sahil's `Treatment_Pages_Contents.docx`.** His clinical content is used near-verbatim throughout; these four credential/claim points were corrected on the way in, and he should see this list. **(1) ROOT CANAL — applied.** His draft: *"Dr. Aishwarya, our in-house endodontic specialist"*, and later *"under a specialist's care"*. In Indian practice "specialist" denotes an MDS; Dr. Aishwarya holds a BDS with advanced endodontic training. Both instances corrected to *"who has advanced training in endodontics (root canal treatment)"*. Clinical substance (digital X-rays, rotary endodontics, single-sitting protocols) untouched. **(2) ORTHODONTICS — pending, page not yet built.** His draft: *"our orthodontists offer…"* — the clinic has no orthodontist (C10). Will be written as *"At Aesthedent we offer metal braces, ceramic braces and clear aligners…"*, silent on the performer. **(3) WISDOM TOOTH — pending.** *"Painless wisdom tooth removal"* is an outcome claim; H1/title will read *"comfortable, minimally invasive"*, with "painless" only as lower-body search phrasing. No performer named (C11). **(4) DENTURES — pending.** His draft reintroduces the **BPS (Biofunctional Prosthetic System)** protocol name, which Phase 4D stripped for lack of certification on file (C9). Technique will be described — muscle-movement impressions, bite registration, facial bow — without the branded name. **[NEEDS-INPUT: is Dr. Sahil BPS-certified? If yes, supply the certificate and the name goes back in.]** |
...
| T3 | ~~`/privacy` and `/terms` are linked from the sitewide footer but don't exist~~ → ✅ **FOOTER LINKS REMOVED 2026-08-08**, immediately before submitting for indexing: the footer renders on all 22 pages, so this was 44 live links into 404s and Googlebot's first real crawl would have spent budget on them. **Privacy and terms pages still need real, reviewed legal copy** — I have not drafted it and should not, unreviewed. **Re-add both footer links once the pages exist.** Deliberately NOT replaced with placeholder pages: thin legal content is its own problem. |
