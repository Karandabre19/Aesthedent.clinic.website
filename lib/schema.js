 ...
 *
 * `mainEntityOfPage` (Phase 4F): pass a page URL to say "this page is about this
 * business". Used by /dental-clinic-in-kothrud.
 *
 * Note what this deliberately does NOT do: it does not mint a second @id or a
 * second `url`. There is one clinic, so there must be one entity - emitting a
 * second Dentist node for the area page would tell Google there are two
 * businesses at one address, which is worse than having no schema on the page.
 * Same @id, same canonical `url` (the homepage), different mainEntityOfPage.
 */
export function buildClinicSchema({ mainEntityOfPage } = {}) {
  return {
...
/**
 * MedicalWebPage for a service page (Phase 4G).
 *
 * Declares the page itself as health content and ties it back to the ONE clinic
 * entity via `about`/`provider` rather than minting a second business node.
 * `lastReviewed` is deliberately omitted: it asserts that a clinician reviewed
 * the page on a date, and inventing that on health content is exactly the kind of
 * unverifiable claim this project keeps stripping out. Add it when Dr. Sahil
 * actually signs off a page, with the real date.
 */
export function buildMedicalWebPageSchema(service) {
  ...
  return {
    ...
    '@id': `${url}#webpage`,
    url,
    name: `${service.title} in ${service.locality}`,
